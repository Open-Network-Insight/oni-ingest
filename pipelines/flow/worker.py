...
class Worker(object):
    ...
    def start(self):
        ...
        self._logger("Listening topic:{0}".format(self.kafka_consumer.Topic))
        for message in self.kafka_consumer.start():
            self._new_file(message.value)

    def _new_file(self,file):

        self._logger("-------------------------------------- New File received --------------------------------------")
    ...
    def _process_new_file(self,file):
        ...
        self._logger("Processing file: {0}".format(process_cmd))
        ...
        self._logger"Creating staging: {0}".format(create_staging_cmd))
        ...
        self._logger("Moving data to staging: {0}".format(mv_to_staging))
        ...
        self._logger( "Loading data to hive: {0}".format(load_to_avro_cmd))
        ...
        self._logger("Removing staging path: {0}".format(rm_hdfs_staging_cmd))
        ...
        self._logger("Removing files from local staging: {0}".format(rm_local_staging))
        Util.execute_cmd(rm_local_staging,self._logger)

        self._logger("File {0} was successfully processed.".format(file_name))
