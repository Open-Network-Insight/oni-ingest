    ...
    def _process_new_file(self,file):
      ...
      #  subprocess.call(process_cmd,shell=True)
       ...
       # subprocess.call(mv_to_staging,shell=True)
        ...
        #load to avro
        load_to_avro_cmd = "hive -hiveconf dbname={0} -hiveconf y={1} -hiveconf m={2} -hiveconf d={3} -hiveconf h={4} -hiveconf data_location='{5}' -f oni/dns/load_dns_avro_parquet.hql".format(self._db_name,flow_year,flow_month,flow_day,flow_hour,hdfs_staging_path)
